...
{ 
	...
	for (int i = 0; i < 192; i++)
	{ 
			buffer_input_data[i]=convert_analog_channel_data(0);
	
			if(ms_per_div>0.5)
			{if(ms_per_div==1) _delay_us(49);
			 if(ms_per_div==1.5) _delay_us(112);
			if(ms_per_div==2) _delay_us(174);}
...
void calc_vpp_vmid(void)
{
	...
	V_max = buffer_input_data[0];
	V_min = buffer_input_data[0];
	...
	for(int i = 1;i<192;i++)
	{ if(V_max < buffer_input_data[i])
		{ V_max = buffer_input_data[i];}
	  if(V_min > buffer_input_data[i]) 
...
freq =8.000/((end_wave_idx-start_wave_idx)*(ms_per_div));
...
void draw_waveform(void)
{
   ...
   for (int x = 0; x < 95; x++)
   {
	   y1 = pixel_loc[x] + vert_disp;				
	   y2 = pixel_loc[x + 1] + vert_disp;			
